...
const Encounter4 = {
  SequenceDissection: function () {
    ...
    for (const position in state) {
      ...
      state[position].toRemove = function (symbol) {
        ...
        for (let component of this.outside) {
          if (
            component != this.insideW1 &&
            component != this.insideW2 &&
            component != symbol
          ) {
            return component;
          }
      ...
      state[position].outsideSolved = function () {
        return (
          this.insideW1 + this.insideW2 === this.outside ||
          this.insideW2 + this.insideW1 === this.outside
        );
      };
      ...
      state[position].innerSolved = function () {
        ...
        return (
          this.insideW1 != this.inside &&
          this.insideW2 != this.inside &&
          leftover === ""
        );
      };
    }
    ...
    if (prev === "left") {
      stepsI.push({
        type: "instruction",
        text: `${Localization.dictionary.left} ${
          Localization.dictionary.take
        } ${this._toIcon(leftT.right)} 
                    ${Localization.dictionary.deposit} ${
          Localization.dictionary.right
        }.`,
      });
      ...
      stepsI.push({
        ...
        text: `${Localization.dictionary.left} ${
          Localization.dictionary.take
        } ${this._toIcon(leftT.middle)} 
                    ${Localization.dictionary.deposit} ${
          Localization.dictionary.middle
        }.`,
      });

      stepsI.push({
        type: "instruction",
        text: `${Localization.dictionary.middle} ${
          Localization.dictionary.take
        } ${this._toIcon(middleT.left)} 
                    ${Localization.dictionary.deposit} ${
          Localization.dictionary.left
        }.`,
      });
      ...
      stepsI.push({
        type: "instruction",
        text: `${Localization.dictionary.right} ${
          Localization.dictionary.take
        } ${this._toIcon(rightT.middle)} 
                    ${Localization.dictionary.deposit} ${
          Localization.dictionary.middle
        }.`,
      });
    ...
    if (
      state.left.hasSameAsStatue() &&
      state.middle.hasSameAsStatue() &&
      state.right.hasSameAsStatue()
    ) {
      ...
      state.left.hasWallDouble() &&
      state.middle.hasWallDouble() &&
      state.right.hasWallDouble()
    ) {
    ...
    stepsI.push({
      type: "infoW1",
      text: `3D Key = ${Localization.dictionary.left}: ${this._toIcon(
        state.left.insideW1 + state.left.insideW2
      )}, 
                ${Localization.dictionary.middle}: ${this._toIcon(
        state.middle.insideW1 + state.middle.insideW2
      )}, 
                ${Localization.dictionary.right}: ${this._toIcon(
        state.right.insideW1 + state.right.insideW2
      )}`,
    });

    if (
      !state.left.innerSolved() ||
      !state.middle.innerSolved() ||
      !state.right.innerSolved()
    ) {
    ...
    stepsO.push({
      type: "info",
      text: `${Localization.dictionary.left}: ${this._toIcon(
        state.left.outside
      )}, 
                ${Localization.dictionary.middle}: ${this._toIcon(
        state.middle.outside
      )}, 
                ${Localization.dictionary.right}: ${this._toIcon(
        state.right.outside
      )}`,
    ...
    while (true) {
      if (
        state.left.outsideSolved() &&
        state.middle.outsideSolved() &&
        state.right.outsideSolved()
      ) {
        break;
      }
      count = count + 1;
  ...
  _toIcon: function (component) {
    switch (component) {
      case "C":
        return `<i class="symbol-sm circle"></i> (${Localization.dictionary.circle})`;
      case "S":
        return `<i class="symbol-sm square"></i> (${Localization.dictionary.square})`;
      case "T":
        return `<i class="symbol-sm triangle"></i> (${Localization.dictionary.triangle})`;
...
$(function () {
  ...
  $(".se-btn-group button").on("click", function () {
    ...
    $(`.se-btn-group[data-area="${area}"] button:not(.btn-light)`).each(
      function () {
